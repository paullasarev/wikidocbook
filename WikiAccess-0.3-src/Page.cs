...
namespace WikiTools.Access
{
	...
	public partial class Page : IPage
	{
        ...
        string xmlText; bool xmlTextLoaded = false;
		...
		public Page(IWiki wiki, string pgname)
		{
			this.wiki = wiki;
			name = pgname;
			ab = wiki.ab;
        ...
        #region XmlText Property

        /// <summary>
        /// Returns page xml text
        /// </summary>
        ...
        public void LoadXmlText()
        {
            try
            {
                xmlText = ab.DownloadPage("index.php?action=raw&title=" + ab.EncodeUrl(name));
            }
            catch (WebException we)
            {
                if (((HttpWebResponse)we.Response).StatusCode == HttpStatusCode.NotFound)
