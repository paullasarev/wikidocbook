...
Node newNodeN (NodeType newType, char* name, char* value, int copyName, int copyValue)
{
    ...
    result->data.nameval->name  = copyName  ? _strdup (name)  : name;
    result->data.nameval->value = copyValue ? _strdup (value) : value;
    return result;
}
...
void outputNode (Node node)
{
    ...
    rname =
        ...
        /* Fallback value */
        (sprintf_s (defaultname, sizeof(defaultname), "type%dnode", node->type), defaultname);
	...
	if (outNL||outNLafter) fb_write_to_buffer("\n");
}
...
void outputXMLHelper (Node node)
{
    ...
    {
        case Heading:
            sprintf_s (tmpstr, sizeof(tmpstr), "<heading level='%d'>", node->data.num);
        ...
        case ExtensionToken:
            ...
            sprintf_s (tmpstr, sizeof(tmpstr), "<extension name='%s'>", nv->name);
            fb_write_to_buffer (tmpstr);
        ...
        case Attribute:
            sprintf_s (tmpstr, sizeof(tmpstr), "<attr name='%s'", node->data.attrdata->name);
            fb_write_to_buffer (tmpstr);
